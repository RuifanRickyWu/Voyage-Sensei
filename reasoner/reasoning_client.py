...
class ReasoningClient:
    ...
    def keyword_summarization_for_poi(self, state_manager: StateManager, poi_list: list[str]):
        template = self._load_prompt_poi_keyword(poi_list)
        keyword_summary_result = json.loads(self._llm_agent.make_request(template))


    def _load_prompt_poi_keyword(self, poi_list: list[str]):
        template = template = self._env.get_template(self._prompt_config.get("POI_KEYWORD_SUMMARIZATION"))
